...
function gmSetTitlePage(title)
{
	document.title = title;
	GmApi.gmUpdateTitle(title);
}
...
function gmHandleAppDisplayed(app_installed, app_to_display)
{
    ...
    {
         GmApi.gmShowApp(app_data.title, app_data.pkg, app_data.activity, app_data.apkurl);
    }
...
function gmHandleAppClick(title, pkg, activity, apkurl, bannerUrl, webUrl, category)
{
    if(category=="web")
    {
    ...
    else if(!GmApi.gmIsBlueStacksInstalled())
    {
        ...
        GmApi.gmShowApp(title, pkg, activity, apkurl);
    }
...
function setLocalName(localeName)
{
    try
    {
        window.gAllStrings = {};
...
function getLocalisedString(label)
{
